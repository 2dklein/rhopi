...
def search(obj, type):
    rho.date = ephem.now()  # This is used to calculate current observatory time/date
    obj.compute(rho)  # This calculates the position of the object based on RHO
...
# Read_db() pulls the user added object data from objects.txt and adds them into the system as a new pyephem object.
def read_db(name):
    with open('objects.txt', 'r') as f:  # Read only the user objects file
        lines = f.readlines()  # Input all of the lines as a list
        for line in lines:  # Search through list of lines
            if line[0:len(name.get())] == name.get():  # find the object that matches the name the user searches
                obj = ephem.readdb(line)  # create pyephem object
                search(obj, 'GET User Object')  # Call search function on this object
                break  # Stop searching through list since we found the object


# track_s() enables and disables 'tracking', or continually updating the object's hour angle
def track_s(name):
    if o_track.get() == 1:  # if the tracking was just turned on
        log_action('Track Object', name.get(), 'Tracking Started')  # Log action
        quick_s(name, True)  # Call quick search again on the object name to trigger the if condition in search()
        c_0['state'] = 'disabled'  # Disable the quick search buttons
        c_1['state'] = 'disabled'  # If they aren't disabled, the user could click another object
        c_2['state'] = 'disabled'  # Which creates a tracking error where two objects are alternatively displayed
        c_3['state'] = 'disabled'
    else:  # If tracking was stopped
        log_action('Track Object', name.get(), 'Tracking Stopped')  # Log action
        if c_fill[0].get() != '--None--':  # If there is an object present, enable the quick search button
            c_0['state'] = 'normal'
        if c_fill[1].get() != '--None--':  # If there isn't, leave it disabled
            c_1['state'] = 'normal'
...
# Quick_s() takes the name on the button and sends it to the appropriate search function
def quick_s(name, track):
    if not track:  # Disable tracking first to prevent a bug
        clear_lock()
    if hasattr(ephem, name.get()):  # Check if it's a named planet/moon
        obj = getattr(ephem, name.get())()  # Get object if it is
        search(obj, 'GET Planet/Moon')  # Send to search function with log text
    else:
        try:
            ephem.star(name.get())  # Check if named star
        except KeyError:
            read_db(name)  # If not, search the name from the custom user objects database
        else:
            obj = ephem.star(name.get())  # If it's a star
            search(obj, 'GET Named Star')  # Send named star to the search function with log text


def clear_lock():
    o_track.set(0)  # Turn the tracking off


# Validate() is used in multiple places to confirm that the number entered in an entry is an int of the correct length
...
def view_large():
    ...
    large_font = tkFont.Font(size=80)
    medium_font = tkFont.Font(size=42)
    box.attributes('-zoomed', True)
    box.title('RhoPi Large View')
    box_f = tk.Frame(box, bg='black')
    box_f.grid(row=0, sticky='WE')
    tk.Label(box_f, text='INFORMATION', relief='solid', width=27).grid(row=0)  # Section Title
    ...
    box_o = tk.Frame(box, bg='black')
    box_o.grid(row=1, sticky='WE')
    tk.Label(box_o, text='', width=50).grid(row=0, columnspan=2)
    ...
    for child in box_o.winfo_children():
...
# print tkFont.families()  # For finding system fonts
...
o = tk.Frame(root)  # o is for object section widgets
