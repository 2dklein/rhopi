...
import time, datetime, ephem, tkFont
...
def confirm_align(name, ra, dec):
    log_action('Aligned', name.get(), str('RA: '+ra+' | DEC: '+dec))
...
def use(name, ra_t, dec_t):
    ...
    log_action('Custom Search', name, str('RA: '+ra+' | DEC: '+dec))  # Log custom object use
    db = ephem.readdb(name+',f,'+ra+','+dec+',0,2000,0')  # Create temp fixed object in ephem database format
    search(db, 'Custom Object')  # Run search and calculate details
...
def search(obj, type):
    ...
    if o_track.get() == 1:
        ...
        log_action(type+' Search', obj.name, 'HA: '+ha)
