...
def save_db(root, name_t, ra_t, dec_t):
    ...
    def save(name, ra_t, dec_t, mag, spec):
        ...
        try:
            ...
            name_err.set('Name cannot be blank!')
            saved.set('Error')
        for ra in ra_t:
            try:
                float(ra.get())
        ...
        if all(error.get() == '' for error in errors):
            ...
            for err in errors:
                if err is not '':
                    tk.Label(error_popup, text=err.get()).pack()
    ...
    tk.Label(d, textvariable=saved, width=12).grid(row=5, column=0, sticky='W')
